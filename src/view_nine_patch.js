...
export default function() {
    ...
    if (selection.length !== 1) {
        ui.message(i18n('select_one_nine_patch_asset'));
        return;
    }
    ...
    if (sk.isGroup(layer)) {
        if (layer.layers.length === 2 && sk.getLayerByNameFromParent('patch', layer) && sk.getLayerByNameFromParent('content', layer)) {
            ...
            if (patch.layers.length >= 4 && slice && sk.isSlice(slice)) {
                ...
                ui.message(i18n('nine_patch_layer_structure_is_wrong'));
                return;
            }
        } else {
            ui.message(i18n('select_one_nine_patch_asset'));
    ...
    const options = {
        identifier: 'view_nine_patch.webview',
        width: 600,
        height: 400,
        show: false,
        title: i18n('view_nine_patch'),
        resizable: false,
        minimizable: false,
    ...
    webContents.on('did-finish-load', () => {
        ...
        ].forEach(key => langs[key] = i18n(key));
    ...
    browserWindow.loadURL(require('../resources/view_nine_patch.html'));
};